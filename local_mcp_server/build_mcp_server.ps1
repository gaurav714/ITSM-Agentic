...
start "LocalDiagnosticMcpServer" "%~dp0LocalDiagnosticMcpServer.exe" --host 127.0.0.1 --port 8765
...
'@ | Set-Content -LiteralPath $DiagPath -Encoding ASCII
    ...
    Get-ChildItem -LiteralPath (Join-Path $Root 'dist_onedir') -Filter 'LocalDiagnosticMcpServer*.zip' |
        Remove-Item -Force

    $ZipPath = Join-Path $Root 'dist_onedir\LocalDiagnosticMcpServer.zip'
    Compress-Archive -Path $BundleDir -DestinationPath $ZipPath -Force

    $BuildDir = Join-Path $Root 'build'
    $SpecPath = Join-Path $Root 'LocalDiagnosticMcpServer.spec'
    ...
    Write-Host 'Built MCP server zip:'
    Write-Host $ZipPath
}
finally {
    Pop-Location
