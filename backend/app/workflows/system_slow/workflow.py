...
class SystemSlowWorkflow(BaseWorkflow):
    ...
    def handle(
        ...
        if state in ("idle", None) or session.get("workflow") != self.workflow_id:
            ...
            return self._start_local_agent_diagnostics(session)
    ...
    # -- Helpers -------------------------------------------------------------
    def _start_local_agent_diagnostics(
        ...
        return AgentMessageResponse(
            message=(
                ...
                "the local diagnostic assistant running on this system and collect diagnostics — one moment."
            ),
            workflow=self.workflow_id,
            ...
            cards=[
                UICard(
                    ...
                    data={
                        ...
                        "method": "local_diagnostic_agent",
                        "status": "collecting_local_agent_metrics",
        ...
        if self._edge_stop_available(diagnostic):
            ...
            return AgentMessageResponse(
                message=(
                    ...
                    "Reply 'yes' to close Edge using the local diagnostic assistant, "
    ...
    def _handle_remediation_confirmation(
        ...
        if decision == "confirm":
            ...
            return AgentMessageResponse(
                message="I'll ask the local diagnostic assistant to close Microsoft Edge now.",
                workflow=self.workflow_id,
                state="awaiting_remediation_action",
                metadata={
                    "trigger_local_agent_action": {
                        "action": "stop_edge",
    ...
    def _handle_remediation_action_result(
        ...
        if not result:
            return AgentMessageResponse(
                message=(
                    "I did not receive a result from the local diagnostic assistant. "
